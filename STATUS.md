...
- PHASE-0-ORDER [IN PROGRESS] The accepted first slice covered Books 5.1 and 5.3. Books 5.2, 5.4, and 5.5 are complete in source and local tests. Books 5.6 and 5.7 remain NOT STARTED. Every Book 5.8 item is enumerated below.
...
- B3.R1 [DONE] Auth — `GET /api/auth/status` at `src/index.tsx:507`; `POST /api/auth/setup` at `src/index.tsx:517`; `POST /api/auth/login` at `src/index.tsx:532`; `POST /api/auth/logout` at `src/index.tsx:560`.
- B3.R2 [DONE] State/enforcement — `GET /api/state` at `src/index.tsx:1221`; `POST /api/tick` at `src/index.tsx:1228`; protected `POST /internal/jobs/enforcement` at `src/index.tsx:1258`.
- B3.R3 [DONE] Blocks/appeals/load — `POST /api/blocks/:id/log` at `src/index.tsx:1275`; `POST /api/appeals` at `src/index.tsx:1326`; `GET /api/appeals` at `src/index.tsx:1381`; `POST /api/load-reductions/:id/answer` at `src/index.tsx:1392`.
- B3.R4 [DONE] Predictions — `POST /api/predictions` at `src/index.tsx:1410`; `GET /api/predictions` at `src/index.tsx:1423`; `POST /api/predictions/:id/resolve` at `src/index.tsx:1430`; `GET /api/predictions/calibration` at `src/index.tsx:1445`.
- B3.R5 [DONE] Debriefs — `POST /api/debrief` at `src/index.tsx:1478`; `GET /api/debriefs` at `src/index.tsx:1519`.
- B3.R6 [DONE] Campaign — `GET /api/campaign` at `src/index.tsx:1560`; `POST /api/units/:id/step` at `src/index.tsx:1578`.
- B3.R7 [DONE] Maxims/cards — `GET /api/maxims` at `src/index.tsx:1670`; `POST /api/maxims` at `src/index.tsx:1676`; `POST /api/maxims/:id/my-words` at `src/index.tsx:1689`; `GET /api/cards/due` at `src/index.tsx:1710`; `POST /api/cards/:maximId/review` at `src/index.tsx:1721`.
- B3.R8 [DONE] Flags — `POST /api/flags/:id/ack` at `src/index.tsx:1755`; `GET /api/flags/history` at `src/index.tsx:1764`.
- B3.R9 [DONE] Tongue — `POST /api/tongue` at `src/index.tsx:1784`; `GET /api/tongue` at `src/index.tsx:1807`; `PUT /api/tongue/:id` at `src/index.tsx:1828`; `DELETE /api/tongue/:id` at `src/index.tsx:1840`; `GET /api/tongue/due` at `src/index.tsx:1852`; `POST /api/tongue/:id/review` at `src/index.tsx:1873`; `GET /api/tongue/exam` at `src/index.tsx:1931`; `POST /api/tongue/exam/submit` at `src/index.tsx:1941`; `GET /api/tongue/stats` at `src/index.tsx:1965`.
- B3.R10 [DONE] Laws/rewards/stats — `GET /api/laws` at `src/index.tsx:2007`; `POST /api/laws/:id/check` at `src/index.tsx:2017`; `GET /api/rewards` at `src/index.tsx:2038`; `POST /api/rewards/:id/redeem` at `src/index.tsx:2042`; `GET /api/stats` at `src/index.tsx:2072`.
- B3.R11 [DONE] Intel — `GET /api/intel` at `src/index.tsx:2175`; `POST /api/intel` at `src/index.tsx:2191`; `POST /api/intel/:id/verdict` at `src/index.tsx:2211`; `POST /api/intel/:id/analyze` at `src/index.tsx:2479`.
- B3.R12 [DONE] Library/calendar — `GET /api/library` at `src/index.tsx:2243`; `POST /api/library/:bookId/chapter/:idx` at `src/index.tsx:2257`; `GET /calendar.ics` at `src/index.tsx:2313`.
- B3.R13 [DONE] Hermes — `POST /api/hermes` at `src/index.tsx:2394`; `GET /api/hermes/history` at `src/index.tsx:2440`; `POST /api/hermes/council` at `src/index.tsx:2448`.
- B3.R14 [DONE] Agent lifecycle — retired `GET /api/agent/token` at `src/index.tsx:2515` and retired `POST /api/agent/token/rotate` at `src/index.tsx:2518`; owner-session-only credential issuance at `src/index.tsx:2525`, safe metadata listing at `src/index.tsx:2556`, and per-credential revocation at `src/index.tsx:2581`.
- B3.R15 [DONE] Versioned agent operations — POST-only briefing, pending blocks, debrief reads, intel reads/writes, debrief writes, block logs, Hermes messages, and export are at `src/index.tsx:2601-2773`. Read-like operations accept an empty JSON body because credential usage/rate/audit accounting writes D1; unsupported GET/HEAD methods are rejected before agent authentication so they remain zero-write.
- B3.R16 [DONE] Shell/PWA — `GET /manifest.json` at `src/index.tsx:2775`; `GET /sw.js` at `src/index.tsx:2781`; `GET /` at `src/index.tsx:2806`.
...
- B3.X2 [DONE] `README.md` still describes production as pending and lists eight tabs. Its schema inventory has been reconciled to 31 tables across six migrations; production state remains evidence-sensitive and repository work claims no deployment.
...
- B5.3.2 [DONE — SOURCE] Shared enforcement is `src/index.tsx:1246`, browser invocation remains `POST /api/tick` at `src/index.tsx:1228`, and protected internal invocation is `src/index.tsx:1258`.
...
- B17.9 [DONE — IMPLEMENTATION COMMITTED, DOC COMMIT/PUSH PENDING] Book 5.5 source, migration, tests, frontend credential interface, and safe bridge passed the evidence recorded in B5.5 and are committed as `6b37725`. Independent specification and quality reviewers both failed with provider HTTP 429 before returning findings, so no reviewer approval is claimed. The separate documentation commit and branch push remain before this repository slice is closed. Phase 0 remains IN PROGRESS because Books 5.6-5.7 and uncovered Book 5.8 acceptance items remain open.
